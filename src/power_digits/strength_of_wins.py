...
OUTPUT_DIR = ROOT / "data" / "processed" / "strength_of_wins"
OUTPUT_FILE = OUTPUT_DIR / f"strength_of_wins_{SEASON}.parquet"
...
def main():
    ...
    # Current-season completed wins by FBS teams.
    ...
    # ------------------------------------------------------------------
    # The team being evaluated caused the opponent's loss in every
    # Strength of Wins game.
    #
    # Therefore we remove the caused loss from the opponent's
    # WeightedLosses.
    #
    # We do NOT remove anything from WeightedWins because the team being
    # evaluated did not cause a win for that opponent in this game.
    # ------------------------------------------------------------------

    caused_losses = caused.select([
    ...
    #
    # The caused-loss amount is associated with the team being evaluated,
    # so it is subtracted from each opponent's losses.
    # ------------------------------------------------------------------

    strength = (
        games.group_by(["team_id", "team"])
        .agg([
            pl.col("opponent_weighted_wins")
            .sum()
            .alias("weighted_wins"),
